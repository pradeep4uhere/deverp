...
class Category extends Model
{
   //
    ...
    public function getSubCategoryList($category_id){
     ...
     public function getAllStoreTypeCategoryWithChild($sty){
        $cateArr=array('9999'=>'Not Available');
        $catObj = Category::where('store_type','=',$sty)->where('parent_id','=','0')
                ->where('status','=','1')
                ->get();
                //dd($catObj);
        foreach ($catObj as $obj) {
     ...
     public function getAllStoreTypeSubCategoryWithChild($sty,$parent_id){
        $cateArr=array('-1'=>'Not Available');
        $catObj = Category::where('store_type','=',$sty)->where('parent_id','=',$parent_id)
                ->where('status','=','1')
                ->get();
                //dd($catObj);
        foreach ($catObj as $obj) {
